    ...
    public ResponseHandlerInterface getResponseHandler() {
        return new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                debugResponse(LOG_TAG, String.format(Locale.getDefault(),"Response of size: %d", responseBody == null ? 0 : responseBody.length));
            }

            @Override
            public void onProgress(long bytesWritten, long totalSize) {
                addView(getColoredView(LIGHTRED, String.format(Locale.getDefault(),"Progress %d from %d", bytesWritten, totalSize)));
            }
